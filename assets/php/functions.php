...
$db = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME) or die("database isn't connected");
...
// FUNCTION FOR SHOWING THE formdata
...
// for checking duplicate email id in database
...
// for checking duplicate username in database
...
function validateLoginForm($form_data)
{
    ...
    return $response;  // this response has three things: status , username , password;
}


// for checking user that user exist in the database or not
...
function createUser($data)
{
    ...
    $query .= "VALUES('$firstName','$lastName',$gender,'$email','$username','$password','0')";

    // debugging here to get error if there is some error
...
function validateUpdateForm($form_data, $image_data)
{
    ...
    if (isUsernameRegisteredByOther($form_data['username'])) {
        $response['msg'] = $form_data['username'] . " username is already register";
        $response['status'] = false;
        $response['field'] = 'username';
    }
    ...
    if ($image_data['name']) {
        ...
        if ($type != 'jpg' && $type != 'jpeg' && $type != 'png') {
            $response['msg'] = "only jpg,jpeg and png extension are allowed";
            $response['status'] = false;
...
function updateProfile($data, $imagedata)
{
    ...
    if ($imagedata['name']) {
        ...
        $target_dir = __DIR__ . "/../images/profile/" . $image_name;
        move_uploaded_file($imagedata['tmp_name'], $target_dir);
...
// for validating addpost image only
...
function validatePostImage($image_data)
{
    ...
    if (!$image_data['name']) {
        $response['msg'] = "image field is empty";
        $response['status'] = false;
...
{
    global $db;
    $text = mysqli_real_escape_string($db, $data['post_text']);

    $user_id = $_SESSION['userdata']['id'];

    $image_name = time() . basename($image['name']);
    echo $image_name;
    $target_dir = __DIR__ . "/../images/posts/" . $image_name;

    move_uploaded_file($image['tmp_name'], $target_dir);
    $query = "INSERT INTO posts(user_id,post_text,post_img)";
    $query .= "VALUES('$user_id','$text','$image_name')";
...
{
    global $db;
    // here we are joining two table and fetching data
    $query = "SELECT posts.id,posts.user_id,posts.post_img,posts.post_text,posts.created_at,users.id as uid,users.first_name,users.last_name,users.username,users.profile_pic FROM posts JOIN users ON users.id=posts.user_id ORDER BY posts.id DESC";
    $run = mysqli_query($db, $query);
    if (!$run) {
        var_dump($run);
    }
...
//  function for getting userdata using usename
...
// for getting users for follow suggetions
...
// for filtering the suggetions list
...
// for cheching the user  is followed by current user or not

function checkFollowStatus($user_id)
{
    global $db;
    $current_id = $_SESSION['userdata']['id'];
    $query = "SELECT count(*) as row FROM follow_list WHERE follower_id=$current_id && user_id=$user_id";
    $run = mysqli_query($db, $query);
...
// function for getting all likes on a post
...
// function for adding the comments to the database 
...
// getting all the post on my profile page
...
// function for new message notification 
...
// getting all messages of chatted users
