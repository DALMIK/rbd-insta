...
if (isset($_GET['getmessages'])) {
    ...
    foreach ($chats as $chat) {
        ...
        $chatlist .= '  
        <div style="cursor:pointer;" class="d-flex justify-content-between border-bottom chatlist_item" data-bs-toggle="modal" data-bs-target="#chatbox" onclick="popchat(' . $chat['user_id'] . ')" >
    ...
    if (isset($_POST['chatter_id']) && $_POST['chatter_id'] != 0) {
        ...
        foreach ($messages as $cm) {
            if ($cm['from_user_id'] == $_SESSION['userdata']['id']) {
                $cl1 = 'align-self-end bg-white border-dark text-dark';
                $cl2 = 'text-muted ';
            } else {
                $cl1 = 'border-primary';
                $cl2 = 'text-muted';
        ...
        $json['chat']['msgs'] = '<div class="spinner-border text-center" role="status">Loading...
    </div>';
    }
